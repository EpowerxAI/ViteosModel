# -*- coding: utf-8 -*-
"""
Created on Mon Feb  1 20:57:13 2021
...
if(len(list_instance_ids) != 0):
    if(extract_meo == 'yes'):
        query_for_meo_data = coll_2_for_data_in_list_instance_ids.find({ 
    #                                                                         "LastPerformedAction": {"$ne" : 31} #AUA
        ...
        meo_df.to_csv('\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\meo_data\\meo_df_setup_' + setup_code + '_penultimate_date_time_' + date_input + '.csv')

    if(extract_aua == 'yes'):
        query_for_aua_data = coll_2_for_data_in_list_instance_ids.find({ 
                                                                             "LastPerformedAction": {"$ne" : 31} #AUA
#                                                                            "LastPerformedAction": 31 #MEO
																			 ,"TaskInstanceID" : {"$in" : list_instance_ids}
        ...
        aua_df.to_csv('\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\aua_data\\aua_df_setup_' + setup_code + '_penultimate_date_time_' + date_input + '.csv')
...
#
#    if(extract_meo == 'yes'):
#        coll_3_for_writing_in_ml_server = db_3_for_writing_in_ml_server['MEO_RecData_' + setup_code + '_' + date_input]
##        coll_3_for_writing_in_ml_server.insert_many(list_of_dicts_query_result_2)
#    
#    
#        query_4_for_data = coll_2_for_data_in_list_instance_ids.find({ 
#    #                                                                         "LastPerformedAction": {"$ne" : 31} #AUA
#                                                                             "LastPerformedAction": 31 #MEO
#                                        
#                                                                     },
...
#        meo_df = meo_df.loc[:,meo_df.columns.str.startswith('ViewData')]
#        meo_df.to_csv('\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\meo_data\\meo_df_setup_' + setup_code + '_date_' + date_input + '.csv')
#
#    if(extract_aua == 'yes'):
...
#        aua_df = aua_df.loc[:,aua_df.columns.str.startswith('ViewData')]
#        aua_df.to_csv('\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\aua_data\\aua_df_setup_' + setup_code + '_date_' + date_input + '.csv')
#
#
#
#
