...
date_to_analyze = '02022021'
penultimate_date_to_analyze = '01022021'
...
def comgen(x,y,z,k,m,a,b,c):
    ...
    if m in trade_ttype:
        ...
        if ((a!= 0) & (b!= 0)):
            com = 'No2 position break, Geneva to reflect jpm trade on ' + str(z) + " " + 'for price'+' '+ str(a) + ' and quantity' +' ' + str(b)
        elif  ((a==0) & (b!=0)):
...
def common_matching_engine_single2(df,filters,columns_to_output, amount_column, dummy_filter,serial_num,param_setup_code = setup_code,param_client = client):
    dummy = df.groupby(filters)[amount_column].apply(list).reset_index()
    dummy1 = df.groupby(filters)['ViewData.Side0_UniqueIds'].count().reset_index()
    dummy = pd.merge(dummy, dummy1 , on = filters, how = 'left')
    dummy2 = df.groupby(filters)['ViewData.Side1_UniqueIds'].count().reset_index()
...
# In[2428]:
    ...
    filters = ['ViewData.Task Business Date','ViewData.Mapped Custodian Account','ViewData.Currency','ViewData.ISIN']
    serial_num = 11
    df11 = common_matching_engine_double1(df10,filters,columns_to_output, amount_column, dummy_filter,serial_num)
...
# In[2428]:
    ...
    filters = ['ViewData.Task Business Date','ViewData.Mapped Custodian Account','ViewData.Currency','ViewData.Investment ID']
    serial_num = 15
    df15 = common_matching_engine_double1(df14,filters,columns_to_output, amount_column, dummy_filter,serial_num)
...
# In[2429]:
    ...
    serial_num = 17
    df17 =  df16.copy()
    df18 = common_matching_engine_single1(df17,filters,columns_to_output, amount_column, dummy_filter,serial_num)
    serial_num = 18
    df19 = common_matching_engine_single2(df18,filters,columns_to_output, amount_column, dummy_filter,serial_num)
    serial_num = 19
    df19 = common_matching_engine_double1(df19,filters,columns_to_output, amount_column, dummy_filter,serial_num)
    serial_num = 20
    df20 = df19.copy()
...
# In[2430]:
    ...
    serial_num = 21
    df22 = common_matching_engine_single1(df21,filters,columns_to_output, amount_column, dummy_filter,serial_num)
    serial_num = 22
    df23 = common_matching_engine_single2(df22,filters,columns_to_output, amount_column, dummy_filter,serial_num)
    
    filters = ['ViewData.Task Business Date','ViewData.Mapped Custodian Account','ViewData.Currency','ViewData.Description']
    serial_num = 23
    df24 = common_matching_engine_double1(df23,filters,columns_to_output, amount_column, dummy_filter,serial_num)
    serial_num = 24
...
# In[2431]:
    ...
    serial_num = 25
    df26 = common_matching_engine_single1(df25,filters,columns_to_output, amount_column, dummy_filter,serial_num)
    serial_num = 26
    df27 = common_matching_engine_double1(df26,filters,columns_to_output, amount_column, dummy_filter,serial_num)
...
# In[2432]:
    ...
    serial_num = 27
    df28 = common_matching_engine_single1(df27,filters,columns_to_output, amount_column, dummy_filter,serial_num)
    serial_num = 29
    df29 =  df28.copy()
    df30 = common_matching_engine_double1(df29,filters,columns_to_output, amount_column, dummy_filter,serial_num)
...
#    com_temp = pd.read_csv('\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + str(client) +'\\output_files\\Setup_' + str(setup_code) + '\\Lombard comment template for delivery.csv')
    ...
    com_temp = pd.read_csv('\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + str(client) +'\\output_files\\Setup_' + str(setup_code) + '\\Lombard ' + str(setup_code) + ' comment template for delivery.csv')
