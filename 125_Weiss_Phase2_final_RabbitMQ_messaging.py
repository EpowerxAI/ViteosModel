...
#        ReconDF.columns = ['TaskID', 'csc','ReconPurpose','collection_meo','RequestId']
        ReconDF.columns = ['TaskID', 'csc','ReconPurpose','collection_meo','ProcessID','Completed_Status','Setup_Code']
        
        ReconDF['TaskID'] = ReconDF['TaskID'].str.lstrip("b'")
        ReconDF['RequestId'] = ReconDF['RequestId'].str.replace(r"[^0-9]"," ")
        ...
        for i in range(ReconDF.shape[0]):
        #for setup_code in setup_code_list:
            TaskID_i = ReconDF['TaskID'].iloc[i]
            print('Starting predictions for Weiss, setup_code = ' + str(setup_code) + ' and Task ID = ', str(TaskID_i))
            print(setup_code)
