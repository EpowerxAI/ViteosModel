...
cols_for_closed = ['Status','Source Combination','Mapped Custodian Account','Accounting Currency','B-P Currency',\
...
for i in range(0,len(date_numbers_list)):
    ...
    df_153_june_24_meo = pd.read_csv(filepaths_MEO[i])
    
    df_153_june_24_meo = df_153_june_24_meo.drop_duplicates()
    df_153_june_24_meo = df_153_june_24_meo.reset_index()
    df_153_june_24_meo = df_153_june_24_meo.drop('index',1)
    
    meo = df_153_june_24_meo[~df_153_june_24_meo['ViewData.Status'].isin(['SMT','HST', 'OC', 'CT', 'Archive','SMR'])]
    meo = meo[~meo['ViewData.Status'].isnull()]
    meo = meo.reset_index()
    meo = meo.drop('index',1)
    del df_153_june_24_meo
