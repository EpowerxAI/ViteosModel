...
def identifying_closed_breaks_from_Trans_type(fun_side, fun_transaction_type_list, fun_side_meo_combination_df, fun_setup_code_crucial):
    ...
    if(fun_setup_code_crucial == '125' or fun_setup_code_crucial == '170' or fun_setup_code_crucial == '123'):
...
#        elim_col = list(elim1.columns)
        ...
        #        Remove ids containing 'up/down at mapped custodian account' and ids containing 'up/down at currency' from elim. 
            if(len(list_of_side1_ids_to_remove_from_aa_new) != 0):
                ...
                elim_except_mapped_custodian_acct_and_currency = elim[~elim['Side0_UniqueIds'].isin(list_of_side0_ids_to_remove_from_bb_new_without_duplicates)]
                elim_except_mapped_custodian_acct_and_currency.drop_duplicates(keep=False, inplace = True)
...
if(flag_side0_ids_to_remove_from_bb_new_exist == 0 & flag_side1_ids_to_remove_from_aa_new_exist == 0):
    side0_intersection_elim_umr = set(elim['Side0_UniqueIds']) & set(final_umr_table['SideB.ViewData.Side0_UniqueIds'])
    elim_except_mapped_custodian_acct_and_currency_and_umr = elim[~elim['Side0_UniqueIds'].isin(list(side0_intersection_elim_umr))]
    side1_intersection_elim_umr = set(elim['Side1_UniqueIds']) & set(final_umr_table['SideA.ViewData.Side1_UniqueIds'])
...
### Remove Updown IDs
X_test_left = X_test_left[~(X_test_left['SideB.ViewData.Side0_UniqueIds'].isin(updown_ids_b_side))]
...
pb_ids_otm_left = []
acc_id_single = []
if umr_double_count_left.empty == False:
...
if umr_otm_table_double_count.empty == False:
...
if(final_mto_table_copy.empty==False):
    
    final_mto_table_copy['BreakID_Side1'] = meo_df[meo_df['ViewData.Side1_UniqueIds'].isin(list(final_mto_table_copy['SideA.ViewData.Side1_UniqueIds']))]['ViewData.BreakID'].values
...
#final_mtm_table
if(final_mtm_table.empty == False):
...
#Changes made for commenting on 25-11-2020, when new model from Abhijeet came
...
#Changes made for uncommenting on 25-11-2020, when new model from Abhijeet came
...
cols = [
 ...
 'new_pb2','new_pb1'
##Below cols were Removed on 25-11-2020
# ,'comm_marker','monthend marker'
...
##Below com_temp was replaced on 25-11-2020
#filename = 'finalized_model_weiss_catrefine_v10_gompu.sav'

filename = 'finalized_model_weiss_123_v3.sav'
...
com_temp = pd.read_csv('Weiss Comment template for delivery new - 123.csv')
...
if(final_mtm_table.empty == False):
    if(final_df_2[final_df_2['Predicted_action'] == 'UMR_One-Many_to_Many-One'].shape[0] != 0):
        final_df_2_with_UMR_Many_One_to_One_Many = final_df_2[((final_df_2['Predicted_action'] == 'UMR_One-Many_to_Many-One') or (final_df_2['Predicted_action'] == 'UMR_Many_to_Many'))]
    elif(final_df_2[final_df_2['Predicted_action'] == 'UMR_One-Many_to_Many-One'].shape[0] == 0):
        final_df_2_with_UMR_Many_One_to_One_Many = final_df_2[(final_df_2['Predicted_action'] == 'UMR_Many_to_Many')]
else:
    final_df_2_with_UMR_Many_One_to_One_Many = final_df_2[(final_df_2['Predicted_action'] == 'UMR_One-Many_to_Many-One')]
