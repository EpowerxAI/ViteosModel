...
filepaths_aa_new = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\aa_new_setup_' + setup_code + '_date_' + str(date_i) + '_with_X_test_left_with_SPM_sort_df.csv'
aa_new.to_csv(filepaths_aa_new)
...
for key in X_test[~((X_test['SideB.ViewData.Side0_UniqueIds'].isin(exceptions_0_umb_ids)) | \
                    (X_test['SideB.ViewData.Side0_UniqueIds'].isin(final_umt_table['SideB.ViewData.Side0_UniqueIds'])) | \
                    (X_test['SideB.ViewData.Side0_UniqueIds'].isin(final_umr_table['SideB.ViewData.Side0_UniqueIds'])))]\
                    ['SideB.ViewData.Side0_UniqueIds'].unique():
    print(key)
    
    if key in threshold_0_umb:
#        Changes made on 03-12-2020. Change made to sort grouped data to catch otm and mto
        sort_df = X_test[X_test['SideB.ViewData.Side0_UniqueIds']==key].\
                    sort_values(by = 'probability_UMB_2', ascending = [False])
        sort_df = sort_df.reset_index().drop('index',1)
        sort_df = sort_df.loc[0:15,:]
        
        values =  sort_df['SideA.ViewData.B-P Net Amount'].values
        net_sum = sort_df['SideB.ViewData.Accounting Net Amount'].max()

#        Changes made on 03-12-2020. Change made to sort grouped data to catch otm and mto. Below was commented since sort_df took care of it
#        values =  X_test[(X_test['SideB.ViewData.Side0_UniqueIds']==key) & \
#                         (X_test['Predicted_action_2']=='UMB_One_to_One')]['SideA.ViewData.B-P Net Amount'].values
#        net_sum = X_test[X_test['SideB.ViewData.Side0_UniqueIds']==key]['SideB.ViewData.Accounting Net Amount'].max()
        ...
        if subSum(values,net_sum) == []: 
            ...
            id1_aggregation = X_test[(X_test['SideA.ViewData.B-P Net Amount'].isin(amount_array)) & \
                                          (X_test['SideB.ViewData.Side0_UniqueIds']==key)]\
        ...
        values =  X_test[(X_test['SideB.ViewData.Side0_UniqueIds']==key)]\
                                            ['SideA.ViewData.B-P Net Amount'].values
        net_sum = X_test[X_test['SideB.ViewData.Side0_UniqueIds']==key]\
...
#        Changes made on 03-12-2020. Change made to sort grouped data to catch otm and mto
        ...
        sort_df2 = X_test[X_test['SideA.ViewData.Side1_UniqueIds']==key].\
                    sort_values(by = 'probability_UMB_2', ascending = [False])
        sort_df2 = sort_df2.reset_index().drop('index',1)
        sort_df2 = sort_df2.loc[0:15,:]
        
        values2 =  sort_df2['SideB.ViewData.Accounting Net Amount'].values
        net_sum2 = sort_df2['SideA.ViewData.B-P Net Amount'].max()
...
filepaths_mtm_df_eqs = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\mtm_df_eqs_setup_' + setup_code + '_date_' + str(date_i) + '_with_X_test_left_with_SPM_sort_df.csv'
mtm_df_eqs.to_csv(filepaths_mtm_df_eqs)
...
#unpredicted_breakids_1 = get_remaining_breakids(fun_meo_df = meo_df_test, fun_final_df_2 = final_table_to_write_test)
#unpredicted_breakids_2 = get_remaining_breakids_2(fun_meo_df = meo_df_test, fun_final_df_2 = final_table_to_write_test)
...
filepaths_df_to_append = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\df_to_append_setup_' + setup_code + '_date_' + str(date_i) + '_2.csv'
...
#Started at 17:42
filepaths_final_df_2 = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\final_df_2_setup_' + setup_code + '_date_' + str(date_i) + '_with_X_test_left_with_SPM_sort_df_2.csv'
