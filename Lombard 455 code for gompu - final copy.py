...
# create dynamic name with date as folder
base_dir_plus_Lombard_plus_455 = os.path.join(base_dir_plus_Lombard, setup_code)

# create 'dynamic' dir, if it does not exist
if not os.path.exists(base_dir_plus_Lombard_plus_455):
    os.makedirs(base_dir_plus_Lombard_plus_455)
...
def common_matching_engine_single(df,filters,columns_to_output, amount_column, dummy_filter,serial_num):
    ...
    #dummy = pd.merge(dummy, pos , on = ['Custodian Account','Currency','Ticker1'], how = 'left')
        ...
        if df4.shape[0]!=0:
            ...
            filename = 'Lombard/Lombard ' + setup_code + ' ' + string_name + '.csv'
            df4.to_csv(filename)
...
#         k = k[sel_col_1]
#         k.to_csv('prediction result lombard ' + setup_code + ' P1.csv')
...
#             sel_col_1 = ['ViewData.BreakID','ViewData.Side0_UniqueIds','ViewData.Side1_UniqueIds','predicted status','predicted action','predicted category','predicted comment']
#             k = k[sel_col_1]
#             k.to_csv('prediction result lombard ' + setup_code + ' P2.csv')
#             uni5 = uni4[(uni4['amt_marker'] == 0)]
...
#         sel_col_1 = ['ViewData.BreakID','ViewData.Side0_UniqueIds','ViewData.Side1_UniqueIds','predicted status','predicted action','predicted category','predicted comment']
#         k3 = k3[sel_col_1]
#         k3.to_csv('prediction result lombard ' + setup_code + ' P3.csv')
#         uni5 = uni4[(uni4['amt_marker'] == 0)]
...
#             sel_col_1 = ['ViewData.BreakID','ViewData.Side0_UniqueIds','ViewData.Side1_UniqueIds','predicted status','predicted action','predicted category','predicted comment']
#             k3 = k3[sel_col_1]
#             k3.to_csv('prediction result lombard ' + setup_code + ' P4.csv')
#             uni5 = uni4[(uni4['amt_marker'] == 0)]
#             uni5 = uni5.drop(['zero_list', 'len_zero_list', 'remove_mark','amt_marker'], axis = 1)
#             remain_df1 = uni5.copy()
#         else:
...
os.chdir(base_dir)
umb_oto1.to_csv('Lombard/' + setup_code + '/umb lombard ' + setup_code + ' oto.csv')
umb_mtm.to_csv('Lombard/' + setup_code + '/umb lombard ' + setup_code + ' mtm.csv')
umb_otm.to_csv('Lombard/' + setup_code + '/umb lombard ' + setup_code + ' otm.csv')
...
result_non_trade.to_csv('Lombard/' + setup_code + '/Comment file for lombard ' + setup_code + '.csv')
...
final_df_filename_list_inside_Lombard_folder = ['Lombard/Lombard ' + setup_code + ' p' + str(x) + '.csv' for x in [1,2,3,4,5,6]]
...
final_df_filename_list_inside_Lombard_455_folder = ['Lombard/' + setup_code + '/umb lombard ' + setup_code + ' ' + str(x) + '.csv' for x in ['mtm','otm','oto']]
final_df_filename_list_inside_Lombard_455_folder.append('Lombard/' + setup_code + '/Comment file for lombard ' + setup_code + '.csv')
final_df_inside_Lombard_455_folder = check_if_file_exist_in_cwd_and_append_to_df_list_if_exists(final_df_filename_list_inside_Lombard_455_folder)

final_df = final_df_inside_Lombard_folder.append(final_df_inside_Lombard_455_folder)
...
final_df_2['ActionType'] = final_df_2.apply(lambda row : apply_ui_action_column_Lombard(fun_row = row), axis = 1,result_type="expand")            
