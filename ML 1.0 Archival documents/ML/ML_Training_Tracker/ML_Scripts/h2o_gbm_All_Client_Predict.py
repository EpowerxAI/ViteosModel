...
import datetime
import databaseconfig as cfg #Linked to another py file
#import ipdb
...
FIELDS_TO_IGNORE_FILE = "D:/Bhanu/Viteos/devop/Viteos_Status_Reconcilation/ML_Dev_Deployement/ReconML_FieldsToIgnore_withColType_Cash.csv"
...
model ='D:/Bhanu/Viteos/devop/Viteos_Status_Reconcilation/ML_Dev_Deployement/'+csc+ReconPurpose+'.model'
Process_ID = int(args[5])

#========================= Create MongoDB connection ========================================
#ipdb.set_trace()
client = MongoClient(cfg.dbconnection['meo_connection_host'])
#client = MongoClient(cfg.dbconnection['meo_connection_host'],username='appUserRecon',password='V!te$s')
...
try:
    cursor = collection_meo.find( {"TaskInstanceID": TaskId, "ViewData":{"$ne":None}, "MatchStatus":{"$ne":21}, "ViewData.CombinedAndIsPaired" : False}, {'ViewData' : 1.0})
...
df.to_csv("D:/Bhanu/Viteos/devop/Viteos_Status_Reconcilation/ML_Dev_Deployement/temp_predict.csv", index=True, na_rep="", quoting=csv.QUOTE_ALL)

# init H2o and read file
proxyServer="http://192.168.170.200:3128"

h2o.init(proxy = proxyServer, nthreads = 1, max_mem_size='9g')

dfh2o = h2o.import_file(path="D:/Bhanu/Viteos/devop/Viteos_Status_Reconcilation/ML_Dev_Deployement/temp_predict.csv", col_types=new_col_types)
...
#client = MongoClient(cfg.dbconnection['pred_connection_host'],username='appUserRecon',password='V!te$s')
...
dfh = dfh2o[:, ["BreakID", "Client", "ClientShortCode", "CombinedAndIsPaired", "ExternalComment1", "ExternalComment2", "ExternalComment3", "InternalComment1", "InternalComment2", "InternalComment3", "Is Combined Data",
 "Recon Data Model", "Recon Purpose", "Recon Setup", "Recon Setup Code", "Side0_UniqueIds", "Side1_UniqueIds", "Source", "Source Combination", "Source Combination Code", "SourceTypeCode", "Status", "System Comments", "TabID",
 "Task Business Date", "Task ID", "Task Knowledge Date", "Type", "_ID", "ID", "predicted_label"]]
