...
try:
  s2_out = subprocess.check_output([sys.executable, 'D:/Bhanu/Viteos/devop/Viteos_Status_Reconcilation/ML_Dev_Deployement/RMQReceive.py'])
except Exception:
    data = None
...
for i, row in RecomDF.iterrows():    
    ...
    channel.basic_publish(exchange='ReconDEVExchange_ML',
                      routing_key='Recon2_ReadFromML_DEV',
                      body=Message1)
    ...
    try:
        grepOut = subprocess.check_output([sys.executable, 'D:/Bhanu/Viteos/devop/Viteos_Status_Reconcilation/ML_Dev_Deployement/h2o_gbm_All_Client_Predict.py',TaskId, csc, ReconPurpose, collection_meo, Model, RequestId],stderr=STDOUT)
