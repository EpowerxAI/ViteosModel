...
try:
  s2_out = subprocess.check_output([sys.executable, '/home/reconadmin/Viteos_Recon_ML/RECON_ML_SOROS_PROD/RMQReceive.py'])
except Exception:
    data = None
...
for i, row in RecomDF.iterrows():    
    ...
    channel.basic_publish(exchange='ReconPRODSorosExchange_ML',
                      routing_key='Recon2_ReadFromML_PROD_Soros',
                      body=Message1)
    ...
    try:
        grepOut = subprocess.check_output([sys.executable, '/home/reconadmin/Viteos_Recon_ML/RECON_ML_SOROS_PROD/h2o_gbm_All_Client_Predict.py',TaskId, csc, ReconPurpose, collection_meo, Model, RequestId],stderr=STDOUT)
