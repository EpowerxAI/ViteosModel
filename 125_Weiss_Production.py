...
import pika
from ViteosLogger import ViteosLogger_Class

now = datetime.now()
current_date_and_time = now.strftime('%y-%m-%d_%H-%M-%S')


log_folder = 'D:\\ViteosModel\\Logs_for_production_loop\\'
log_filename = 'log_datetime_' + str(current_date_and_time) + '.txt'
log_filepath = log_folder + log_filename

ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = 'Log started for datettime = ' + str(current_date_and_time))
...
ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = 'All function definitions end')
...
ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = 'Mongo DB objects created')
...
ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = 'RabbitMQ Publish object created')

rb_mq_obj_new_for_acknowledgement = rb_mq(param_RABBITMQ_QUEUEING_PROTOCOL = 'amqps', \
                 param_RABBITMQ_USERNAME = 'recon2', param_RABBITMQ_PASSWORD = 'recon2', \
#                 param_RABBITMQ_HOST_IP = 'vitblrmleng01.viteos.com', param_RABBITMQ_PORT = '5671', \vitblresbuat
                 param_RABBITMQ_HOST_IP = 'vu-uat', param_RABBITMQ_PORT = '5671', \
                 param_RABBITMQ_VIRTUAL_HOST = 'viteos', \
                 param_RABBITMQ_EXCHANGE = 'ReconPROD_PARALLELExchange_MLAck', \
                 param_RABBITMQ_QUEUE = 'VNFRecon_WriteToML_Ack_PROD_PARALLEL', \
                 param_RABBITMQ_ROUTING_KEY = 'Recon2_WriteToMLAck_PROD_PARALLEL', \
                 param_test_message_publishing = True, \
                 param_timeout = 10)

ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = 'RabbitMQ ACK object created')
...
while True:
    ...
    ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = 'RabbitMQ Receive file executed')    
...
#string message = string.Format("{0}|{1}|{2}|{3}|{4}|{5}|{6}", task.InstanceID, task.ReconSetupForTask.Client.ClientShortCode, task.ReconSetupForTask.ReconPurpose.ReconPurpose, ReconciliationDataRepository.GetReconDataCollection(task.ReconSetupCode), processID, "Recon Run Completed", task.ReconSetupCode) 
    ...
    if len(stout_list) > 1:
        ...
        ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = 'RabbitMQ Receive file executed and got a message')
...
#        sys.exit(1)
        ...
        ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = 'split happened')
        
        ReconDF = DataFrame.from_records(smallerlist)
        ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = 'ReconDF created')
...
#        ReconDF.columns = ['TaskID', 'csc','ReconPurpose','collection_meo','RequestId']
        ...
        ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = 'ReconDF all columns created')
        print('ReconDF')
        print(ReconDF)
...
#        desired_date_str = desired_date.strftime("%Y-%m-%d")
#        date_input = desired_date_str
        ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = 'Starting for loop')
        for z in range(ReconDF.shape[0]):
        #for setup_code in setup_code_list:
            ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = 'Started for loop')
...
#            rb_mq_obj_new_for_acknowledgement.fun_publish_single_message(param_message_body = AckMessage_z)
            ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = AckMessage_z)
            ...
            if(meo_df.shape[0] == 0):
                ...
                ViteosLogger_Class.log_to_file(param_filename = log_filepath, param_log_str = Message_z)
...
#                            f.write('No no pair ids found for this setup and date combination')
                ...
                if(len(training_df) != 0):
                    ...
                    filename2 = 'Weiss_umt_two_step_loop_1.sav'
