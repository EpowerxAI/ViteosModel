...
path = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\Files and Rules for Lookup\\Rules delivery from Ronson\\Schonfeld\\Schonfeld ML final\\New folder\\'
if((day > 0) & (day < 10)):
    filename = 'Combined Position recon - '+ str(mon) + '0' + str(day) + str(yr)+'.xlsx'
else:
    filename = 'Combined Position recon - '+ str(mon) + str(day) + str(yr)+'.xlsx'
...
df1['Custodian Account'] = df1['Custodian Account'].apply(lambda x : x.strip())
df1['Currency'] = df1['Currency'].apply(lambda x : x.strip())
df1['Ticker1'] = df1['Ticker1'].apply(lambda x : x.strip())

pos['Custodian Account'] = pos['Custodian Account'].apply(lambda x : x.strip())
pos['Currency'] = pos['Currency'].astype(str)
pos['Currency'] = pos['Currency'].apply(lambda x : x.strip())
pos['Ticker1'] = pos['Ticker1'].astype(str)

pos['Ticker1'] = pos['Ticker1'].apply(lambda x : x.strip())

pos['merge_key']
df1_merged = pd.merge(df1, pos, on = ['Custodian Account','Currency','Ticker1'], how = 'left')
df1 = df1.reset_index()
...
#Change made on 06-12-2020. After discussion with Abhijeet, I (Rohit) removed the code written to segregate pairs at the beginning in df2. Instead, now the pair is made in df4 only
...
if df4.shape[0] !=0:
    df4['filter_1'] = df4['Custodian Account'].astype(str) + df4['Currency'].astype(str) + df4['Ticker1'].astype(str) #+ df4['Cash Standing rounded to 0'].astype(str)
    df4['labels_on_filter_1'] = pd.Categorical(df4['filter_1']).codes
    df4_BreakID_list = df4.groupby('labels_on_filter_1')['ViewData.BreakID'].apply(list)
    df4_BreakID_list_dataframe = df4_BreakID_list.to_frame()
    df4_BreakID_list_dataframe.rename(columns = {'ViewData.BreakID' : 'ViewData.BreakID_list'}, inplace = True)
    
    df4_BreakID_list_dataframe['BreakID_to_insert_in_db'],df4_BreakID_list_dataframe['Predicted_BreakID_to_insert_in_db'] = df4_BreakID_list_dataframe['ViewData.BreakID_list'].apply(lambda x : x[0]),df4_BreakID_list_dataframe['ViewData.BreakID_list'].apply(lambda x : x[1:])
    
    df4 = pd.merge(df4,df4_BreakID_list_dataframe,left_on = 'labels_on_filter_1', right_index = True)
    df4.drop(['ViewData.BreakID_list'], axis =1 , inplace = True)
#    df4['Status_to_insert_in_db'] = df4['ViewData.Status'].apply(lambda x : 'UMF' if str(x) == 'SMB' else 'UCB')
    df4['Status_to_insert_in_db'] = 'UMR'
    
    df4['Predicted Comment'] = 'Match'
    df4['Predicted_Status'] = ''
    df4['Predicted_action'] = ''
    df4['PredictedCategory'] = ''
    df4.drop_duplicates(subset = 'BreakID_to_insert_in_db',keep="last", inplace = True)
...
filepaths_final_df_2 = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\final_df_2_setup_' + setup_code + '_date_' + str(date_i) + '_3.csv'
