...
for setup_code in setup_code_list:
    ...
    test_file['Settle_date_diff'] = (pd.to_datetime(test_file['SideA.ViewData.Settle Date'],errors = 'coerce') - pd.to_datetime(test_file['SideB.ViewData.Settle Date'],errors = 'coerce')).dt.days
...
#    umr_mto_table['BreakID_Side0'] = umr_mto_table['SideB.ViewData.Side0_UniqueIds'].apply( \
#                                        lambda x : get_BreakID_from_list_of_Side_01_UniqueIds(fun_meo_df = meo_df, \
#                                                                                              fun_side_0_or_1 = 0, \
#                                                                                              fun_str_list_Side_01_UniqueIds = list(x)))
...
#            umr_mto_table['BreakID_Side0'].iloc[i] = list(meo_df[meo_df['ViewData.Side0_UniqueIds'].isin(umr_mto_table['SideB.ViewData.Side0_UniqueIds'].values[i])]['ViewData.BreakID'])#        fun_otm_mto_df['BreakID_Side1'].iloc[i] = list(fun_meo_df[fun_meo_df['ViewData.Side1_UniqueIds'].isin(fun_otm_mto_df['SideA.ViewData.Side1_UniqueIds'].iloc[i])]['ViewData.BreakID'])
    ...
    umr_mto_table['BreakID_Side1'] = umr_mto_table['SideA.ViewData.Side1_UniqueIds'].apply(lambda x : meo_df[meo_df['ViewData.Side1_UniqueIds'] == x]['ViewData.BreakID'].unique())
...
#        fun_meo_df = fun_meo_df[~fun_meo_df['ViewData.Status'].isin(['SMT','HST', 'OC', 'CT', 'Archive','SMR','SPM'])]
    ...
    df2['ViewData.Settle Date'] = pd.to_datetime(df2['ViewData.Settle Date'], errors = 'coerce')
    df2['ViewData.Trade Date'] = pd.to_datetime(df2['ViewData.Trade Date'], errors = 'coerce')
