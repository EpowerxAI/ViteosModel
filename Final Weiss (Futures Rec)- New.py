...
def identifying_closed_breaks_futures(fun_all_meo_combination_df, fun_setup_code_crucial, fun_trans_type_1, fun_trans_type_2):

    if(fun_all_meo_combination_df.shape[0] != 0):    
        print(fun_trans_type_2)
        print(fun_trans_type_2)

        if(fun_setup_code_crucial.isin(['531','1200','170'])):
...
date_to_analyze = '20012021'
penultimate_date_to_analyze = '19012021'
...
for transaction_type_for_closing_value in mapping_dict_trans_type:
    if(meo_for_closed[meo_for_closed['Transaction_Type_for_closing'] == transaction_type_for_closing_value].shape[0] != 0):
	    ...
	    if(All_combination_df.shape[0] != 0):
	        closed_df_for_transaction_type_for_closing_value = identifying_closed_breaks_futures(fun_all_meo_combination_df = All_combination_df, \
...
closed_df_WITH_DEP_interacting = identifying_closed_breaks_futures(fun_all_meo_combination_df = All_combination_df_WITH_DEP_interacting, \
                                                         fun_setup_code_crucial = setup_code, \
...
for i in keys_to_consider:
    ...
    break_id_0.append(list(sample2[sample2['Trans_side']=='B_side']['ViewData.BreakID'].values))
    break_id_1.append(list(sample2[sample2['Trans_side']=='A_side']['ViewData.BreakID'].values))
    net_amount.append(sample2['ViewData.Net Amount Difference'].sum())
    source_combination.append(sample2['ViewData.Source Combination'].unique()[0])
...
if(len(unique_id_0) != 0):
	...
	final_df['ViewData.Source Combination'] = source_combination
...
final_open_table['ReconSetupName'] = '3011 GS Future Cash Recon'
...
if(final_df.shape[0] != 0):
	...
	final_df['ReconSetupName'] = '3011 GS Future Cash Recon'
