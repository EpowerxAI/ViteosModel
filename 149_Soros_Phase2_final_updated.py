...
df1['ViewData.Status'] = df1['ViewData.Status'].astype(str)
df1['ViewData.Status'] = df1['ViewData.Status'].str.strip()
umb_carry_forward_df = df1[df1['ViewData.Status'] == 'UMB']
...
no_pair_table_to_use_if_test_file_empty = df1[~(df1['ViewData.Status'] == 'UMB')]
no_pair_table_to_use_if_test_file_empty = no_pair_table_to_use_if_test_file_empty[no_pair_table_to_use_if_test_file_empty['ViewData.Status'] == 'OB']
no_pair_table_to_use_if_test_file_empty = no_pair_table_to_use_if_test_file_empty[~(no_pair_table_to_use_if_test_file_empty['ViewData.Side1_UniqueIds'].isin(new_closed_keys))]
no_pair_table_to_use_if_test_file_empty = no_pair_table_to_use_if_test_file_empty[~(no_pair_table_to_use_if_test_file_empty['ViewData.Side0_UniqueIds'].isin(new_closed_keys))]
...
if(len(training_df) != 0):
    ...
    if(len(umr_otm_table)) == 0:
        umr_otm_table = pd.DataFrame()
        
    
    if(len(one_id_0_final) != 0 & umr_otm_table.shape[0] !=0) :
    ...
    X_test_left2 = X_test_left2[~(X_test_left2['SideB.ViewData.Side0_UniqueIds'].isin(filtered_otm_flat))]
    ...
    if X_test_left2[X_test_left2['Predicted_action_2']=='UMB_One_to_One'].empty == False:
        X_test_umb = X_test_left2[X_test_left2['Predicted_action_2']=='UMB_One_to_One']
        X_test_umb = X_test_umb.reset_index().drop('index',1)
    else:
        X_test_umb = pd.DataFrame()
    
    if (len(X_test_umb) != 0) :
        one_side_unique_umb_ids = one_to_one_umb(X_test_umb)
    else:
        one_side_unique_umb_ids = pd.DataFrame()
    
    if (len(one_side_unique_umb_ids) != 0) :
        ...
        final_oto_umb_table = pd.DataFrame()
         
    # ## Removing IDs from OTO UMB
    
    if final_oto_umb_table.empty == False: 
        ...
        X_test_left3 = X_test_left2.copy()
    ...
    if (len(X_test_left3) != 0) :
        open_ids_0_last , open_ids_1_last = no_pair_seg2(X_test_left3)
    else:
        open_ids_0_last = pd.DataFrame()
        open_ids_1_last = pd.DataFrame()
    if len(open_ids_0_last) ==0:
        open_ids_0_last = ['None']
    if len(open_ids_1_last) ==0:
        open_ids_1_last = ['None']
    ...
    if(umr_otm_table_final.shape[0] != 0):
        ...
        umr_otm_table_final_new_to_write = pd.DataFrame()    
    ...
    if(umr_mto_table.shape[0] != 0):    
        ...
        umr_mto_table_new_to_write = pd.DataFrame()
   ...
   umr_otm_table_final = pd.DataFrame()
   umr_mto_table = pd.DataFrame()
   final_oto_umb_table = pd.DataFrame()
   final_umr_table = pd.DataFrame()
   final_no_pair_table_copy = pd.DataFrame()
   final_table_to_write = pd.DataFrame()
...
if(umb_carry_forward_df.shape != 0):
...
if(no_pair_table_to_use_if_test_file_empty.shape[0] != 0):
    no_pair_table_to_use_if_test_file_empty_columns_to_select_from_meo_df = ['ViewData.BreakID', \
                                                       'ViewData.Task Business Date', \
                                                       'ViewData.Side0_UniqueIds', \
                                                       'ViewData.Side1_UniqueIds', \
                                                       'ViewData.Source Combination Code', \
                                                       'ViewData.Task ID']
    no_pair_table_to_use_if_test_file_empty = no_pair_table_to_use_if_test_file_empty[no_pair_table_to_use_if_test_file_empty_columns_to_select_from_meo_df]
    
    no_pair_table_to_use_if_test_file_empty['Predicted_Status'] = 'OB'
    no_pair_table_to_use_if_test_file_empty['Predicted_action'] = 'No-Pair'
    no_pair_table_to_use_if_test_file_empty['ML_flag'] = 'ML'
    no_pair_table_to_use_if_test_file_empty['SetupID'] = setup_code 
    no_pair_table_to_use_if_test_file_empty['Final_predicted_break'] = ''
    no_pair_table_to_use_if_test_file_empty['PredictedComment'] = ''
    no_pair_table_to_use_if_test_file_empty['PredictedCategory'] = ''
    no_pair_table_to_use_if_test_file_empty['probability_UMB'] = ''
    no_pair_table_to_use_if_test_file_empty['probability_No_pair'] = ''
    no_pair_table_to_use_if_test_file_empty['probability_UMR'] = ''
    
    no_pair_table_to_use_if_test_file_empty[no_pair_table_to_use_if_test_file_empty_columns_to_select_from_meo_df] = no_pair_table_to_use_if_test_file_empty[no_pair_table_to_use_if_test_file_empty_columns_to_select_from_meo_df].astype(str)
    change_names_of_no_pair_table_to_use_if_test_file_empty_mapping_dict = {
                                                'ViewData.Side0_UniqueIds' : 'Side0_UniqueIds',
                                                'ViewData.Side1_UniqueIds' : 'Side1_UniqueIds',
                                                'ViewData.BreakID' : 'BreakID',
                                                'ViewData.Task ID' : 'TaskID',
    ...
    no_pair_table_to_use_if_test_file_empty.rename(columns = change_names_of_no_pair_table_to_use_if_test_file_empty_mapping_dict, inplace = True)
    
    no_pair_table_to_use_if_test_file_empty['BusinessDate'] = pd.to_datetime(no_pair_table_to_use_if_test_file_empty['BusinessDate'])
    no_pair_table_to_use_if_test_file_empty['BusinessDate'] = no_pair_table_to_use_if_test_file_empty['BusinessDate'].map(lambda x: dt.datetime.strftime(x, '%Y-%m-%dT%H:%M:%SZ'))
    no_pair_table_to_use_if_test_file_empty['BusinessDate'] = pd.to_datetime(no_pair_table_to_use_if_test_file_empty['BusinessDate'])
    
    no_pair_table_to_use_if_test_file_empty['Side0_UniqueIds'] = no_pair_table_to_use_if_test_file_empty['Side0_UniqueIds'].astype(str)
    no_pair_table_to_use_if_test_file_empty['Side1_UniqueIds'] = no_pair_table_to_use_if_test_file_empty['Side1_UniqueIds'].astype(str)
    no_pair_table_to_use_if_test_file_empty['Final_predicted_break'] = no_pair_table_to_use_if_test_file_empty['Final_predicted_break'].astype(str)
    no_pair_table_to_use_if_test_file_empty['Predicted_action'] = no_pair_table_to_use_if_test_file_empty['Predicted_action'].astype(str)
    no_pair_table_to_use_if_test_file_empty['probability_No_pair'] = no_pair_table_to_use_if_test_file_empty['probability_No_pair'].astype(str)
    no_pair_table_to_use_if_test_file_empty['probability_UMB'] = no_pair_table_to_use_if_test_file_empty['probability_UMB'].astype(str)
    no_pair_table_to_use_if_test_file_empty['probability_UMR'] = no_pair_table_to_use_if_test_file_empty['probability_UMR'].astype(str)
    no_pair_table_to_use_if_test_file_empty['SourceCombinationCode'] = no_pair_table_to_use_if_test_file_empty['SourceCombinationCode'].astype(str)
    no_pair_table_to_use_if_test_file_empty['Predicted_Status'] = no_pair_table_to_use_if_test_file_empty['Predicted_Status'].astype(str)
    no_pair_table_to_use_if_test_file_empty['ML_flag'] = no_pair_table_to_use_if_test_file_empty['ML_flag'].astype(str)
    
    
    no_pair_table_to_use_if_test_file_empty[['BreakID', 'TaskID']] = no_pair_table_to_use_if_test_file_empty[['BreakID', 'TaskID']].astype(float)
    no_pair_table_to_use_if_test_file_empty[['BreakID', 'TaskID']] = no_pair_table_to_use_if_test_file_empty[['BreakID', 'TaskID']].astype(np.int64)
    
    no_pair_table_to_use_if_test_file_empty[['SetupID']] = no_pair_table_to_use_if_test_file_empty[['SetupID']].astype(int)
    filepaths_no_pair_table_to_use_if_test_file_empty = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\no_pair_table_to_use_if_test_file_empty.csv'
    no_pair_table_to_use_if_test_file_empty.to_csv(filepaths_no_pair_table_to_use_if_test_file_empty)
    
    final_table_to_write = final_table_to_write.append(no_pair_table_to_use_if_test_file_empty)
