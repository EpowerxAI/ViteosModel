...
if(client_input == 'Soros'):
    db_name = 'ReconDB_Soros'
else:
    db_name = 'ReconDB'
  
setup_code_input_list = ['125']
date_input_list = ['2020-12-11','2020-12-10','2020-12-09','2020-12-08','2020-12-07','2020-12-06','2020-12-05','2020-12-04','2020-12-03','2020-12-02','2020-12-01']

base_dir_to_write_data_in = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client_input + '\\'
...
db_1_for_instance_ids = mngdb_obj_1_for_instance_ids.client[db_name]
...
db_2_for_data_in_list_instance_ids = mngdb_obj_2_for_data_in_list_instance_ids.client[db_name]

#Comment out below code if data is to be written in some other db as well. Otherwise let it remain as it is
...
#        list_instance_ids = list(df_instance_ids['InstanceID'])
        ...
        instance_ids_temp_path = '//vitblrdevcons01/Raman  Strategy ML 2.0/All_Data/' + client_input + '/Setup_and_Date_wise_Task_Instance_Ids/' + setup_code + '_' + date_input + '.txt'
        with open(instance_ids_temp_path, 'w') as f:
            for item in list_instance_ids:
                f.write("%s\n" % item)

        if(len(list_instance_ids) != 0):
            query_for_meo_data_from_prod = coll_2_for_data_in_list_instance_ids.find({ 
                        "ViewData": { "$ne": None },
            ...
            list_of_dicts_query_for_meo_data = list(query_for_meo_data_from_prod)

            if(len(list_of_dicts_query_for_meo_data) != 0):
                meo_df = json_normalize(list_of_dicts_query_for_meo_data)
                meo_df = meo_df.loc[:,meo_df.columns.str.startswith('ViewData')]
                meo_df.to_csv(base_dir_to_write_data_in + 'meo_data\\meo_df_setup_'+ setup_code + '_date_' + str(date_input) + '.csv')
                del(meo_df)
                del(list_of_dicts_query_for_meo_data)
            else:
                print('No aua data found for ' + setup_code + ' for date ' + date_input)

            query_for_aua_data_from_prod = coll_2_for_data_in_list_instance_ids.find({ 
                        "ViewData": { "$ne": None },
                        "ViewData.Status": { "$nin": ["HST","Archive","OC"] },
                        "TaskInstanceID" : {"$in" : list_instance_ids},
            ...
            list_of_dicts_query_for_aua_data = list(query_for_aua_data_from_prod)

            if(len(list_of_dicts_query_for_aua_data) != 0):
                aua_df = json_normalize(list_of_dicts_query_for_aua_data)
                aua_df = aua_df.loc[:,aua_df.columns.str.startswith('ViewData')]
                aua_df.to_csv(base_dir_to_write_data_in + 'aua_data\\aua_df__setup_'+ setup_code + '_date_' + str(date_input) + '.csv')
                del(aua_df)
                del(list_of_dicts_query_for_aua_data)
            else:
                print('No aua data found for ' + setup_code + ' for date ' + date_input)
                
                
        else:
             instance_ids_temp_path_warning = '//vitblrdevcons01/Raman  Strategy ML 2.0/All_Data/' + client_input +'/Setup_and_Date_wise_Task_Instance_Ids/WARNING_' + setup_code + '_' + date_input + '.txt'
